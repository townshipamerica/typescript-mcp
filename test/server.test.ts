...
const centroidFeature = {
  ...
  geometry: { type: "Point" as const, coordinates: [-110.3, 44.5] as [number, number] },
  properties: {
...
describe("plss_to_coordinates", () => {
  it("returns search result for valid description", async () => {
    ...
    const res = await runPlssToCoordinates(client, { description: "NW 25 24N 1E 6th Meridian" });
  ...
  it("maps 429 quota error to MCP error text", async () => {
    const client = makeClient(vi.fn().mockRejectedValue(new QuotaError("quota exceeded")));
    const res = await runPlssToCoordinates(client, { description: "NW 25 24N 1E 6th Meridian" });
    expect(res.isError).toBe(true);
...
describe("coordinates_to_plss", () => {
  ...
  it("maps 429 to quota error", async () => {
    const client = makeClient(vi.fn().mockRejectedValue(new QuotaError("rate limited")));
    const res = await runCoordinatesToPlss(client, { lat: 44.5, lng: -110.3 });
    expect(res.isError).toBe(true);
...
describe("plss_to_geojson", () => {
  it("returns polygon features only", async () => {
    const client = makeClient(vi.fn().mockResolvedValue(mockFeatureCollection));
    const res = await runPlssToGeojson(client, { description: "NW 25 24N 1E 6th Meridian" });
    expect(res.isError).toBeFalsy();
    const fc = JSON.parse(res.content[0].text) as { features: Array<{ geometry: { type: string } }> };
...
describe("validate_description", () => {
  it("marks a well-formed description as valid", async () => {
    ...
    const parsed = JSON.parse(res.content[0].text) as { valid: boolean; normalized: string };
    expect(parsed.valid).toBe(true);
    expect(typeof parsed.normalized).toBe("string");
  });

  it("marks a garbage string as invalid and provides suggestion", async () => {
    const res = await runValidateDescription(null, { description: "hello world" });
    const parsed = JSON.parse(res.content[0].text) as { valid: boolean; suggestion: string };
...
describe("batch_convert", () => {
  ...
  it("rejects batch over 1000 items", async () => {
    const client = makeClient(vi.fn());
    const res = await runBatchConvert(client, {
      descriptions: Array.from({ length: 1001 }, () => "NW 25 24N 1E 6th Meridian")
    });
    expect(res.isError).toBe(true);
    expect(res.content[0].text).toContain("1,000");
  });

  it("maps 429 to quota error", async () => {
    const client = makeClient(vi.fn().mockRejectedValue(new QuotaError("quota")));
    const res = await runBatchConvert(client, {
...
describe("land_report", () => {
  it("returns coming_soon stub", async () => {
    const res = await runLandReport(null, { description: "NW 25 24N 1E 6th Meridian" });
    expect(res.isError).toBeFalsy();
